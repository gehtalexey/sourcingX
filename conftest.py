...
class MockOpenAIClient:
    ...
    Captures the prompts sent and returns configurable responses.
    """
    ...
    def get_last_prompt(self) -> str:
        """Return the user message from the last API call."""
    ...
    def get_system_prompt(self) -> str:
        """Return the system message from the last API call."""
        if self.captured_calls:
            messages = self.captured_calls[-1].get('messages', [])
            for msg in messages:
...
def mock_openai_client():
    """Provide a mock OpenAI client with default responses."""
    return MockOpenAIClient()


@pytest.fixture
def mock_openai_client_factory():
    """Factory fixture to create mock clients with custom responses."""
...
def strong_backend_profile():
    """A strong backend engineer profile from a top company."""
    return {
        'first_name': 'David',
        'last_name': 'Cohen',
        'name': 'David Cohen',
        ...
        'current_company': 'Wiz',
        'headline': 'Senior Backend Engineer at Wiz',
...
def weak_consulting_profile():
    """A weak profile from a consulting/body shop company."""
    return {
        'first_name': 'John',
        'last_name': 'Doe',
        'name': 'John Doe',
        'current_title': 'Software Developer',
        'current_company': 'Matrix IT',
        'headline': 'Software Developer at Matrix IT',
        'location': 'Tel Aviv, Israel',
        'summary': 'Developer working on various client projects.',
        'skills': ['Java', '.NET', 'SQL Server'],
...
def overqualified_profile():
    """An overqualified profile (VP/Head of level)."""
    return {
        'first_name': 'Sarah',
        'last_name': 'Levi',
        'name': 'Sarah Levi',
...
@pytest.fixture
def empty_profile():
    """An empty/minimal profile that should be skipped."""
    return {
        'first_name': '',
        'last_name': '',
        'name': '',
...
def devops_team_lead_profile():
    """A DevOps Team Lead profile with Kubernetes experience."""
    return {
        'first_name': 'Alex',
        'last_name': 'Stern',
        'name': 'Alex Stern',
        'current_title': 'DevOps Team Lead',
        'current_company': 'Snyk',
        'headline': 'DevOps Team Lead at Snyk',
        'location': 'Tel Aviv, Israel',
...
def backend_job_description():
    """Standard backend engineer job description."""
    return """
...
def devops_team_lead_job_description():
    """DevOps Team Lead job description with specific requirements."""
    return """
    DevOps Team Lead
    Tel Aviv, Israel
...
def fullstack_job_description():
    """Fullstack engineer job description."""
    return """
...
def screening_result_validator():
    """Utility to validate screening result structure."""
...
@pytest.fixture
def profile_builder():
    """Factory to build test profiles with customizable fields."""
    def _build(name="Test User", title="Software Engineer", company="TestCo",
        ...
        profile = {
            ...
            'all_employers': employers or [company],
            'all_titles': [title]
        }
        if include_raw:
