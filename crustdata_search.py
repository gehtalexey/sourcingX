...
# history for profiles the new search endpoints can't return them for.
# Verified live against docs.crustdata.com 2026-07-20: up to 10,000 LinkedIn
# URLs per job, base profile = 1 credit (additive pricing, same as the sync
# /person/enrich — see CREDITS_PER_ENRICH_PROFILE_BASE below).
...
def search_people_db_v2(
    ...
    enrich_thin_profiles_for_batch() in dashboard.py), so this flag is a
    visibility signal, not the only path to getting them filled in.
