...
--
-- *** WHY THIS FILE LIVES IN migrations/manual/, NOT migrations/ ***
...
-- repo's db_migrations.py auto-discovers every file in migrations/ via
-- `migrations_dir.glob('*.sql')` (non-recursive) and has no statement-by-
-- statement, non-transactional execution mode — if a future runner picked
-- this file up as pending and applied it as one transaction, it would fail
-- and silently apply NONE of the statements below, then get permanently
-- stuck on it. Living under migrations/manual/ keeps it out of that glob
-- (Codex review round 2, PR #109) while still keeping the file in git history.
--
