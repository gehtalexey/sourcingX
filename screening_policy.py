...
When a must-have is needs_verification (not mentioned, but not contradicted either), do not lower the score for the missing text. Score the candidate on how strong the visible career is against everything else the brief asks, exactly as if that must-have were met, and write "verify in call: <must-have>" in its evidence. Missing text is never a reason for a score of 6 or below; only a contradiction, a matched exclusion, or a genuinely weak fit on the rest of the brief is.

Cite the specific visible fact behind any inference. Never invent company or team facts.
...
- 5-6: borderline — real gaps against the brief (not just text that is missing) — NO GO
